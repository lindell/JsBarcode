...
const leftSide = (data: string): string => data.substr(0, 4);
const rightSide = (data: string): string => data.substr(4, 4);
...
function encodeGuarded({
	...
	leftEncoded,
	rightEncoded,
}: EncodingData, data: string, options: Options): Encoding[] {
...
function encodeFlat({
	leftEncoded,
	rightEncoded,
}: FlatEncodingData, data: string, options: Options): Encoding {
	return {
