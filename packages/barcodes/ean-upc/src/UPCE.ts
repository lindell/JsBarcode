...
function encode(data: string, options: Options): Encoding | Encoding[] {
...
function flatEncoding(options: Options, upcA: string, middleDigits: string): Encoding {
	var result = '';
...
function encodeMiddleDigits(upcA: string, middleDigits: string): string {
	const numberSystem = upcA[0];
	const checkDigit = upcA[upcA.length - 1];
	const parity = PARITIES[parseInt(checkDigit, 10)][parseInt(numberSystem, 10)];
	return encodeEAN(middleDigits, parity);
}

function guardedEncoding(options: Options, upcA: string, middleDigits: string): Encoding[] {
	var result: Encoding[] = [];

	// Add the UPC-A number system digit beneath the quiet zone
	if (options.displayValue && options.text) {
		result.push({
			data: '00000000',
			text: options.text[0],
...
function expandToUPCA(middleDigits: string, numberSystem: string): string {
	const lastUpcE = parseInt(middleDigits[middleDigits.length - 1], 10);
