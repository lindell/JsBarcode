...
import codabar from '.';
const { encode, valid } = codabar() as any;

describe('Codabar', function() {
	it('should encode a string with start and stop characters', function() {
		assert.equal('10110010010101011001010100101101100101010101101001011010100101001001011', encode('A12345B', {}).data);
	});

	it('should add start and stop characters to a string without them', function() {
		// should encode to "A12345A"
		assert.equal('10110010010101011001010100101101100101010101101001011010100101011001001', encode('12345', {}).data);
	});
